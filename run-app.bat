...
echo ====================================================
start "Campus Connect Metro Bundler" cmd.exe /k "npx expo start --android --localhost --clear"

:: 6. Launch Expo Go on the emulator automatically
echo.
echo [5/5] Launching Expo Go on the emulator...
ping -n 6 127.0.0.1 >nul
adb shell am start -a android.intent.action.VIEW -d "exp://127.0.0.1:8081" >nul 2>&1
adb shell monkey -p com.campusconnect.app -c android.intent.category.LAUNCHER 1 >nul 2>&1
...
echo     If app does not open automatically, use: exp://127.0.0.1:8081
echo.
